...
def get_data(name, split_id, data_dir, height, width, batch_size, workers,
    ...
    train_loader_head = DataLoader(
        Preprocessor(train_set, root="/home/bfs/zty/reid_market/examples/data/cuhk03/images_head",
                     transform=train_transformer),
        batch_size=batch_size, num_workers=workers,
        shuffle=True, pin_memory=True, drop_last=True)
    train_loader_upper = DataLoader(
        Preprocessor(train_set, root="/home/bfs/zty/reid_market/examples/data/cuhk03/images_upper",
                     transform=train_transformer),
        batch_size=batch_size, num_workers=workers,
        shuffle=True, pin_memory=True, drop_last=True)
    train_loader_lower = DataLoader(
        Preprocessor(train_set, root="/home/bfs/zty/reid_market/examples/data/cuhk03/images_lower",
                     transform=train_transformer),
        batch_size=batch_size, num_workers=workers,
        shuffle=True, pin_memory=True, drop_last=True)
    ...
    val_loader_head = DataLoader(
        Preprocessor(dataset.val, root="/home/bfs/zty/reid_market/examples/data/cuhk03/images_head",
                     transform=test_transformer),
        batch_size=batch_size, num_workers=workers,
        shuffle=False, pin_memory=True)
    val_loader_upper = DataLoader(
        Preprocessor(dataset.val, root="/home/bfs/zty/reid_market/examples/data/cuhk03/images_upper",
                     transform=test_transformer),
        batch_size=batch_size, num_workers=workers,
        shuffle=False, pin_memory=True)
    val_loader_lower = DataLoader(
        Preprocessor(dataset.val, root="/home/bfs/zty/reid_market/examples/data/cuhk03/images_lower",
                     transform=test_transformer),
        batch_size=batch_size, num_workers=workers,
        shuffle=False, pin_memory=True)
    ...
    test_loader_head = DataLoader(
        Preprocessor(list(set(dataset.query) | set(dataset.gallery)),
                     root="/home/bfs/zty/reid_market/examples/data/cuhk03/images_head", transform=test_transformer),
        batch_size=batch_size, num_workers=workers,
        shuffle=False, pin_memory=True)
    test_loader_upper = DataLoader(
        Preprocessor(list(set(dataset.query) | set(dataset.gallery)),
                     root="/home/bfs/zty/reid_market/examples/data/cuhk03/images_upper", transform=test_transformer),
        batch_size=batch_size, num_workers=workers,
        shuffle=False, pin_memory=True)
    test_loader_lower = DataLoader(
        Preprocessor(list(set(dataset.query) | set(dataset.gallery)),
                     root="/home/bfs/zty/reid_market/examples/data/cuhk03/images_lower", transform=test_transformer),
        batch_size=batch_size, num_workers=workers,
        shuffle=False, pin_memory=True)
...
#        return
    ...
    if hasattr(model_head.module, 'base'):
        ...
        new_params_head = [p for p in model_head.parameters() if
                      id(p) not in base_param_ids_head]
        param_groups_head = [
            {'params': model_head.module.base.parameters(), 'lr_mult': 0.1},
            {'params': new_params_head, 'lr_mult': 1.0}]
    else:
        param_groups_head = model_head.parameters()
    optimizer_head = torch.optim.SGD(param_groups_head, lr=args.lr,
                                momentum=args.momentum,
                                weight_decay=args.weight_decay,
                                nesterov=True)

    if hasattr(model_head.module, 'base'):
        base_param_ids_upper = set(map(id, model_upper.module.base.parameters()))
        new_params_upper = [p for p in model_upper.parameters() if
                      id(p) not in base_param_ids_upper]
        param_groups_upper = [
            {'params': model_upper.module.base.parameters(), 'lr_mult': 0.1},
            {'params': new_params_upper, 'lr_mult': 1.0}]
    else:
        param_groups_upper = model_upper.parameters()
    optimizer_upper = torch.optim.SGD(param_groups_upper, lr=args.lr,
                                momentum=args.momentum,
                                weight_decay=args.weight_decay,
                                nesterov=True)

    if hasattr(model_lower.module, 'base'):
        base_param_ids_lower = set(map(id, model_lower.module.base.parameters()))
        new_params_lower = [p for p in model_lower.parameters() if
                      id(p) not in base_param_ids_lower]
        param_groups_lower = [
            {'params': model_lower.module.base.parameters(), 'lr_mult': 0.1},
            {'params': new_params_lower, 'lr_mult': 1.0}]
    else:
        param_groups_lower = model_lower.parameters()
    optimizer_lower = torch.optim.SGD(param_groups_lower, lr=args.lr,
                                momentum=args.momentum,
                                weight_decay=args.weight_decay,
                                nesterov=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--width', type=int,
                        help="input width, default: 128 for resnet*, "
                             "56 for inception")
    parser.add_argument('--combine-trainval', action='store_true',default=False,
